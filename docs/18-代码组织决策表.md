...
| 19  | 「登录 token」存到 utils 单例 + LocalStorage              | XSS 可读 + JS 可窃 + 失去响应式                         | 凭证走 httpOnly cookie（前端 JS 不可读）；登录标记 `Session('auth')` 归 `user.ts`，profile/permissions 内存态 |
...
| 路由级布局（layout 私有子组件） | `src/layouts/<m>/components/` | default/components/Header, default/components/Sidebar, portal/components/PortalHeader |
...
├── app.ts       # 侧边栏、语言、全局 loading（persist: layout+locale，key='vue3-vite-project:app-ui'）
├── user.ts      # 登录标记 Session('auth') + profile + 权限（无持久化；凭证走 httpOnly cookie）
├── theme.ts     # 主题模式（pinia persist: key='vue3-vite-project:theme-mode'，经 namespacedStorageKey 拼接）
├── router.ts    # 路由 UI 状态（按需 import，不集中 re-export）
...
| `docs/19-Pinia store使用规范.md`                       | store 决策表 + 持久化 + 单测模板                            |
...
_文档版本：v1.0.1 | 编写日期：2026-09-04 | 配套项目版本：vue3-vite-project 0.x | 修订记录：v1.0.1 (2026-09-04) §7 store 边界 + 反例 19 同步 2026-08-12 httpOnly 改造_
