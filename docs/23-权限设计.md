...
declare module 'vue-router' {
  interface RouteMeta {
    ...
    affix?: boolean // 多页签固定（供远程菜单协议使用；本地首页已排除多页签，勿配此标记）
...
| **空数组语义** | `hasPerm([])` / `hasAnyPerm([])` **返回 `true`**（视为「无要求」） | 守卫入口 / `meta.permissions` 缺失时的兜底（与 `src/router/guards/permission.ts` 行为对齐） |
...
export function useLogout() {
  ...
  async function confirmLogout(): Promise<void> {
    // 二次确认走 useConfirm composable（v1.14.2 改造）：
    //   - 取消 resolve false，业务方无需 try/catch
    //   - 真实异常原样上抛（与 ElMessageBox.confirm 行为差异）
...
> `loggingOut: Ref<boolean>` 用于消费方（Header.vue / PortalHeader.vue）在二次确认 + 退出过程中**禁用退出按钮防重入**——`watch(loggingOut, ...)` 切换 disabled 状态。

````

> 跳转职责在此 composable（2026-08-12 改造）：`store.logout` 不再依赖 router 实例，斩断 `store → router → guards → store` 循环依赖。
>
> **v1.14.2 二次确认迁移**：早期实现为 `ElMessageBox.confirm + try/catch`；2026-08 切换到 `useConfirm` 后调用方无须 try/catch（取消即 `false`），且 `useConfirm` 默认按钮文案 / 危险预设与 `ElMessageBox` 行为对齐。详见 `docs/27-ProDialog使用指南.md §8.5`。
