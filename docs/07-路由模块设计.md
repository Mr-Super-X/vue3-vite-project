...
> ⚠️ **2026-07-24 审计发现**：Sidebar 菜单渲染**尚未实现**。`src/components/layout/Sidebar.vue` 当前仅 18 行（标题 + 折叠按钮），即使路由已自动注册 + 远程菜单已加载，侧边栏不会自动出现菜单项。本文档描述的是"路由 + 远程菜单加载层"的完整能力，但 UI 渲染层需要单独实现。详见 `docs/audit/2026-07-24-usability-review.md` P0-1。
