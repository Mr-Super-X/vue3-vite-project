...
└── router/
    ...
    ├── auto-register.ts                  # import.meta.glob 扫描各模块 routes/
...
| **定义层** | `router/types.ts` + `router/component-registry.ts`                                               | 路由 name 集中定义 + 组件映射           |
...
// 3. 在 router/modules/preview.ts 定义路由
// 4. 在 component-registry.ts 追加映射
```
...
### 步骤 3：在 `component-registry.ts` 追加映射

```ts
export const COMPONENT_REGISTRY = {
  // ... 已有
  OrderList: () => import('@/modules/order/views/List.vue'), // ← 新增
}
```

### 步骤 4（自动注册版）：在 `src/modules/order/routes/index.ts` 声明路由
...
| 模式       | 路由来源                                                            | 新增路由流程             |
| ---------- | ------------------------------------------------------------------- | ------------------------ |
| **local**  | `src/modules/**/routes/index.ts` 自动注册                           | 步骤 1-5 全部完成即可    |
| **remote** | `src/modules/**/routes/index.ts`（兜底）+ 后端 `/api/menu` 动态注入 | 步骤 1-5 + 后端菜单 JSON |
...
```
□ 1. 新增路由是否同时更新 types.ts / component-registry.ts？
...
- 源码：
  ...
  - `src/router/auto-register.ts`（自动注册：扫描 src/modules/**/routes/index.ts）
  - `src/router/config.ts`（菜单模式配置）
  - `src/router/whitelist.ts`（白名单）
  - `src/router/types.ts`（RouteName 联合类型）
  - `src/router/component-registry.ts`（组件映射）
