...
> ✅ Sidebar 菜单渲染已在 v3 实现（2026-07-24）：从 `router.getRoutes()` 自动派生 + 多级递归 + i18n 标题解析 + 折叠态联动。详见 `src/layouts/default/components/Sidebar.vue`（347 行，含设计取舍注释）与 `docs/audit/2026-07-24-v3-usability-review.md` §1.1 P0-1。
...
> **注意**：`RouteName` 在 `src/router/types.ts` 已简化为 `string`（2026-07-24 方案 A），因此白名单集合的元素类型是 `string`，不再用联合类型约束。dev 模式从 `@/modules/demo/routes` 派生并 spread 进白名单，prod 构建时整个 `...(import.meta.env.DEV ? ... : [])` 被 Rollup tree-shake 掉，不会进入生产包。
...
```ts
// 1. 在 router/whitelist.ts 加入白名单
export const ROUTE_WHITE_LIST: ReadonlySet<string> = new Set<string>([
  'Login',
  'Forbidden',
  'NotFound',
  'ServerError',
  // 'Preview', // ← 跳过登录 + 权限（业务路由加白名单）
])

// 2. 在 modules/preview/routes/index.ts 定义路由
...
业务模块路由的 **第一层** 节点（path = `/xxx`）必须指定 layout。本项目提供 3 套：
...
| **portal**  | `@/layouts/portal/index.vue`  | 顶部水平导航（PortalHeader + PortalNav）+ 主内容区，无侧栏 | 门户 / 对外展示站 / 公开站点首页               | 通常 true（白名单） |

> **典型搭配**：
>
> - 业务页（首页 / UserList / Orders）：`/home` 等 → `default`
> - 登录页：`/login` → `blank` + 加白名单
> - 门户首页：`/portal` 等 → `portal` + 加白名单（适合无侧栏的对外站点）
...
### 2️⃣ （无需在 `types.ts` 追加）RouteName 已简化为 `string`

```ts
// src/router/types.ts（无需改动）
export type RouteName = string
```

> 2026-07-24 方案 A 已移除 RouteName 联合类型——单一事实源是 `src/modules/<feature>/routes/index.ts`，
> 拼写校验靠 zod runtime（remote.ts）+ `pushByNameStrict` dev 模式 throw。新增业务路由**无需改 types.ts**。
...
> **关于 component-registry**：原 `src/router/component-registry.ts` 已合并到 `src/router/auto-register.ts`，`COMPONENT_REGISTRY` 由自动扫描到的 `autoRegisteredRoutes` 派生。2026-07-24 方案 A 进一步移除 `RouteName` 联合类型（`src/router/types.ts` 简化为 `string`），因此新增业务路由从"types.ts + component-registry.ts + routes/index.ts" 三处改动降为仅 `routes/index.ts` **一处改动**，`scripts/check-routes.ts` 也同步移除 component-registry 与 RouteName 联合校验项。
...
// 在 setup 中：
const { pushByName, replaceByName, pushByNameStrict, pushWithTitle, back, addDynamicRoute } =
  useAppRouter()

// 按 name 跳转（name: string —— 2026-07-24 方案 A 后 RouteName 已简化为 string，
// 类型约束靠 §"zod runtime 校验" + pushByNameStrict dev throw 兜底）
await pushByName('UserList', { id: 1 })

// 严格模式：dev 模式下路由未注册/未注入时 throw Error（生产环境由 remote.ts zod 兜底）
// 适合「宁可早失败也不要静默跳错页面」的关键路径
await pushByNameStrict('UserList', { id: 1 })
...
| 方法                                                           | 说明                                      | 适用场景                       |
| -------------------------------------------------------------- | ----------------------------------------- | ------------------------------ |
| `pushByName(name, params?)`                                    | 按 name 跳转，失败统一 toast              | 默认入口                       |
| `replaceByName(name, params?)`                                 | 按 name 替换当前路由（不产生历史）        | 表单提交后跳转避免返回重复填写 |
| **`pushByNameStrict(name, params?)`**                          | dev 模式 throw，prod 退化 pushByName 行为 | 关键业务路径（支付、提单）     |
| `pushWithTitle(to)`                                            | 跳转 + 写 `document.title`（含 i18n）     | 普通业务跳转                   |
| `back(fallback?)`                                              | 安全返回（history 为空时 fallback）       | 表单取消 / 详情返回            |
| `addDynamicRoute(route)`                                       | 运行时动态追加路由（remote menu 用）      | 远程菜单加载                   |
| `goHome() / goLogin(returnUrl?) / go403() / go404() / go500()` | 5 个快捷跳转                              | 错误页 / 登录页                |
| `withErrorToast(fn)`                                           | 包装任意异步函数，自动 toast 错误         | 业务方需要统一错误处理         |
...
```
□ 1. （已废止）types.ts RouteName 联合类型已移除（2026-07-24 方案 A），无需追加
